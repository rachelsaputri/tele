...
async function startalpha() {
    bot.on('callback_query', async (alpha) => {
        ...
        const username = user.username ? user.username : "TioXd";
        ...
        try {
            switch (action[0]) {
                case "menucmd": {
                    let hit_total;
                    try {
                        hit_total = await simple.fetchJson('https://api.countapi.xyz/hit/botak.biz.id/visits')
                    } catch {
                        hit_total = {
                            value: "-"
                        }
